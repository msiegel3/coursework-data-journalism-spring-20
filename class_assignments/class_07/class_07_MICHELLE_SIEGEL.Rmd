...
pills_population_county_west_virginia <- pills_population %>%
  filter(statefips==54 & buyer_state=="WV" & year %in% c(2006:2014)) %>%
...
geo_pills_population_county_west_virginia <- county_geodata_shifted %>%
 inner_join(pills_population_county_west_virginia, by=c("GEOID" = "countyfips"))

# Step 6: Draw the facet map
geo_pills_population_county_west_virginia %>%
...
# Pills per person shipped to Mingo County rose from 2006 to 2008, peaked in 2008, then fell from 2008 to 2014.
...
# Step 4: Filter the pills population table just for West Virginia and create the pills per person metric. 
pills_population_county_wv_kt <- pills_population %>%
  filter(statefips %in% c(21,54)  & buyer_state %in% c("WV","KY") & year %in% c(2006:2009)) %>%
  group_by(countyfips, buyer_county, buyer_state, year) %>%
  summarise(total_pills=sum(dosage_unit, na.rm = TRUE),
            total_population=sum(population, na.rm=TRUE)) %>%
  mutate(pills_per_person=total_pills/total_population)
...
# Step 5: Join your pills per person table to a county level shapefile
geo_pills_population_county_wv_kt <- county_geodata_shifted %>%
 inner_join(pills_population_county_wv_kt, by=c("GEOID" = "countyfips"))

# Step 6: Draw the facet map
geo_pills_population_county_wv_kt %>%
  ggplot(aes(fill = pills_per_person)) +
  facet_wrap(~year, nrow=2) +
  geom_sf(lwd = 0) +
  theme_map() +
