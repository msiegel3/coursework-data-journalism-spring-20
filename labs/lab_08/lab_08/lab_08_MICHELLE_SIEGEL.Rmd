...
average_pills_county_url <- 'https://raw.githubusercontent.com/smussenden/spring20-data-journalism/master/labs/lab_05/data/county_pills_per_year.csv'

average_pills_county_downloaded <- read_csv(average_pills_county_url)
...
state_estimated_population_white_alone <- get_acs(geography = "state", 
...
opioid_states_scrape <- read_html("https://www.cdc.gov/drugoverdose/data/analysis.html") %>%
  html_nodes('table') %>%
  html_table(header=1, fill=TRUE)  %>%
  as.data.frame()
