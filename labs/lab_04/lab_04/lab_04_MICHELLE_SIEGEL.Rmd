...
mingo %>%
  ...
  summarise(shipments = n(),
            ...
            avg_pills_per_shipment = mean(dosage_unit),
            min_pills_in_shipment = min(dosage_unit),
            max_pills_in_shipment =
max(dosage_unit),
            median_pills_in_shipment = median(dosage_unit)
            ) %>%
  arrange(desc(median_pills_in_shipment))
...
# The most significant combo is manufacturer Actavis Pharma, Inc., distributor MIAMI-LUKEN and pharmacy TUG VALLEY PHARMACY, LLC, with 4954200 total pills reaching Mingo County, West Virginia via this pipeline.
...
mingo_working <- mingo %>%
  ...
  group_by(reporter_name, reporter_address_complete) %>%
  summarise(total_pills = sum(dosage_unit))
...
# How can I use a case_when() function to clean data so that in an existing particular column (in this case, "reporter_name"), all of the cells that begin with a certain string of characters (in this case, "CARDINAL HEALTH," which would include "CARDINAL HEALTH", "CARDINAL HEALTH 110, LLC" AND "CARDINAL HEALTH, INC.") are changed to contain "CARDINAL HEALTH" only?
