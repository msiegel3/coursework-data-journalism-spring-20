...
# The majority of records that show up in the pills table, but not the population table pertain to U.S. territories. Without an inner join to get rid of these rows -- that is, if you wanted them to be factored in -- data analyses will be complicated by the fact that any entry for the territories will lack information.
...
geo_pills_population_state_2009 %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="West Virginia had the most pills per person in 2009", subtitle = "High ratios also found in KY and TN, among other states", caption = "Source: DEA ARCOS database, via Washington Post") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Pills per person increase from 2006 to 2012 seen in most states nationwide", subtitle = "WV, KY among those hit hardest by opioid epidemic", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
pills_population_county_2009 <- pills_population %>%
  filter(year==2009) %>%
  group_by(countyfips, buyer_county) %>%
  summarise(total_pills=sum(dosage_unit, na.rm = TRUE),
            total_population=sum(population, na.rm=TRUE)) %>%
  mutate(pills_per_person=total_pills/total_population)
...
# Beyond Appalachia, pills per person figures seem higher in the West.
...
pills_population_county_maryland <- pills_population %>%
  filter(statefips==24 & buyer_state=="MD" & year %in% c(2006:2012)) %>%
...
geo_pills_population_county_maryland <- county_geodata_shifted %>%
 inner_join(pills_population_county_maryland, by=c("GEOID" = "countyfips"))

```

```{r}

geo_pills_population_county_maryland %>%
  ggplot(aes(fill = pills_per_person)) +
  facet_wrap(~year, nrow=2) +
  geom_sf(lwd = 0) +
  theme_map() +
...
# In most Maryland counties, pills per person figures increase from 2006 to 2010-2011, then decline in 2012.
