...
# rows: 290,908
# columns: 42
# dbl, drug code, and transaction id all could be stored as <chr>, because we would never do math with them
# null
...
# NEWCARE HOME HEALTH SERVICES, with 6,133,600 total pills
...
# MCKESSON CORPORATION, with 58,267,330 pills
...
baltimore %>%
  ...
  group_by(reporter_name, buyer_name, buyer_dea_no, buyer_city, buyer_address1) %>%
  summarise(shipments = n(),
            total_pills = sum(dosage_unit)
            ) %>%
  arrange(desc(shipments))

# Professional Pharmacy Service, with 3,352,600 pills

# It is based at Sinai Hospital
